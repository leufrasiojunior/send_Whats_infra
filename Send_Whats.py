...
def generate_bases():
    ...
    try:
        ...
        messagebox.showerror(
            "Arquivo não salvo!",
            "Falha no processo para salvar o arquivo de exemplo em "
            + xlsx_personalizado,
        )
        my_logger.warning(
            f"error when saving file to folder {xlsx_personalizado} %s", str(SE)
        )
...
def text_os():
    try:
        ...
        messagebox.showerror(
            "Erro ao salvar!",
            "Falha no processo para salvar a configuração!: " + str(TG),
        )
...
def texto_personalizado():
    ...
    try:
        for nome in nomes:
            ...
            btnYes = customtkinter.CTkButton(
                windowPerson, text="Sim", command=btnyes
            ).place(relx=0.2, rely=0.2)

            btnNo = customtkinter.CTkButton(
                windowPerson, text="Não", command=btnno
        ...
        messagebox.showerror(
            ...
            "Falha no processo para carregar o arquivo! Verifique a variável" + str(TP),
        )
...
def save_config():
    ...
    saveConf = {"time_send": tab, "time_close_tab": time}
...
def config():
    ...
    aviso = customtkinter.CTkLabel(
        config,
        text="Configure os tempos para a aplicação funcionar antes de enviar a mensagem e a próxima mensagem.\nSempre com o tempo em segundos.",
        justify="center",
    )
    ...
    label = customtkinter.CTkLabel(
        config, text="Tempo para aguardar o envio de mensagens.", justify="center"
    )
    ...
    lbl_close_tab = customtkinter.CTkLabel(
        config, text="Tempo para enviar a próxima mensagem", justify="center"
    )
    lbl_close_tab.pack()
...
def load_base():
    ...
    if qtdNomes > 0:
        customtkinter.CTkLabel(
            app,
            text=f"Nomes a processar: {qtdNomes}                           ",
            fg_color="#DBDBDB",
        ).place(x=225, y=50)
...
def questBases():
    ...
    r1 = customtkinter.CTkRadioButton(
        questbases, text="Aguardando Pagamento", variable=radio, value=1
    ).place(x=80, y=35)

    r2 = customtkinter.CTkRadioButton(
        questbases, text="        O.S         ", variable=radio, value=2
    ).place(x=270, y=35)

    r3 = customtkinter.CTkRadioButton(
        questbases, text="Texto Customizado   ", variable=radio, value=3
    ).place(x=400, y=35)
...
image_label = customtkinter.CTkLabel(
    app, image=img_exa, text="", corner_radius=8
).place(x=55, y=53)
...
label = customtkinter.CTkLabel(
    app, text="Nenhuma base selecionada ainda.", fg_color="#DBDBDB"
).place(x=225, y=50)
app.update()
...
btnSend = customtkinter.CTkButton(
    app, text="Enviar Mensagens!", command=send_messages, image=send_img
).place(x=95, y=210)
...
example_base = customtkinter.CTkButton(
    app, text="Gerar base de Exemplo", command=generate_bases, image=img_exa
).place(x=150, y=5)

img_config = customtkinter.CTkImage(Image.open("images/config.png"))

btnconfig = customtkinter.CTkButton(
    app, text="", command=config, image=img_config, height=20, width=20
).place(x=5, y=5)
